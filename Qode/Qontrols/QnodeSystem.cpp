...
QnodeSystem::QnodeSystem(const IRECT& _bounds, Qurve& _qurve, const Qheme& _qheme, IActionFunction _actionFunq)
  ...
  , qnodeDiameter(_bounds.H() * 0.056f) // kuru....
  , qurve(_qurve)
  , qheme(_qheme)
  ...
  if (_mod.L)
  {
    if (downQnodeIndex != -1)
    {
      ...
      if (downQnodeIndex == 0 || downQnodeIndex == qurve.getQnodeCount() - 1)
      {
        ...
        snapToGrid(newY);
        qurve.setQnode(first, {0.0f, newY});
        qurve.setQnode(last, {1.0f, newY});
      }
      else
      {
    ...
    else if (downQurveIndex != -1)
    {
      ...
      if (_mod.A)
      {
        ...
        newY1 = Clip(newY1, 0.0f, 1.0f);
        newY2 = Clip(newY2, 0.0f, 1.0f);
...
void QnodeSystem::OnMouseDblClick(float _x, float _y, const IMouseMod& _mod)
{
  ...
  {
    qurve.updateCurrentState();
    qurve.setQurvature(overQurveIndex, 0);
    qurve.qreateQurve();
    SetDirty();
...
{
  _qui.FillRect(qheme.foreGround.WithOpacity(0.00943f), mRECT);

  if (overQurveIndex != -1)
    _qui.FillRect(qheme.foreGround.WithOpacity(0.0436f), getQurveSeqt(overQurveIndex));
}

//*****************************************************************************************************************************************
void QnodeSystem::drawQurve(IGraphics& _qui)
{
  _qui.DrawData(qheme.foreGround.WithOpacity(0.73f), mRECT, qurve.getQurveBuffer(), maxResolution + xtraValues, nullptr, 0, strokeWidth * 0.75f);
}
...
void QnodeSystem::drawQnodes(IGraphics& _qui)
{
  ...
  for (int i = 0; i < qnodeCount; i++)
  {
    ...
    if (downQnodeIndex == i)
    {
      _qui.DrawRoundRect(qheme.foreGround.WithOpacity(0.67f), pairToRect(std::pair{x, y}), 2.81f, 0, strokeWidth * 0.75f);
      _qui.FillRoundRect(qheme.foreGround.WithOpacity(0.67f), pairToRect(std::pair{x, y}).GetPadded(-2), 2.81f);
    }
    else if (overQnodeIndex == i)
      _qui.DrawRoundRect(qheme.foreGround, pairToRect(std::pair{x, y}), 2.81f, 0, strokeWidth * 0.75f);
    else
      _qui.DrawRoundRect(qheme.foreGround.WithOpacity(0.67f), pairToRect(std::pair{x, y}), 2.81f, 0, strokeWidth * 0.75f);
  }
}
...
{
  float x = (_pair.first * mRECT.W() + mRECT.L) - (qnodeDiameter / 2);
  float y = (_pair.second * mRECT.H() + mRECT.T) - (qnodeDiameter / 2);
...
IRECT QnodeSystem::getQurveSeqt(int _index) const
{
  ...
  return IRECT::MakeXYWH(lower, mRECT.T, upper - lower, mRECT.H());
}