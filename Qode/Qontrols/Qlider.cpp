...
void Qlider::Draw(IGraphics& _qui)
{
  ...
  drawQlider(_qui, qheme.foreGround.WithOpacity(smoothGlow));

  if (isRising && smoothGlow < 1.f)
    SetDirty(false);
  else if (!isRising && smoothGlow > .67f)
    SetDirty(false);
}
...
//*****************************************************************************************************************************************
void Qlider::OnMouseOver(float, float, const IMouseMod&)
{
  if (!isRising)
  {
    isRising = true;
