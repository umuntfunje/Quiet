...
void QornerResizer::drawResizer(IGraphics& _qui, const IColor& _color, bool _isDragging)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    for (int j = 0; j < 3; j++)
    {
      ...
      if (_isDragging)
        _qui.DrawRoundRect(_color, IRECT::MakeXYWH(r.L + size * i, r.T + size * j, size, size).GetScaledAboutCentre(.6f), .5f, 0, .73f);
      else
        _qui.FillRoundRect(_color, IRECT::MakeXYWH(r.L + size * i, r.T + size * j, size, size).GetScaledAboutCentre(.6f), .5f);
    }
  }
}