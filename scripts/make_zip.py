...
def  main():
  ...
  installer = "\installer\Quiet Installer.exe"
   
  if demo:
    installer = "\installer\Quiet Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Quiet manual.pdf" 
  ]
  ...
  ZIPNAME = "Quiet-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Quiet-v" + FULLVERSIONSTR + "-win-demo.zip"
