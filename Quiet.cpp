...
#include "Qode/Qontrols/Qnob.h"
#include "Qode/Qontrols/Qlider.h"
#include "Qode/Qontrols/Qoggle.h"
#include "Qode/Qontrols/UndoQutton.h"
#include "Qode/Qontrols/RedoQutton.h"
#include "Qode/Qontrols/QnodeSystem.h"
#include "Qode/Qontrols/QurvePresets/Qenu.h"
#include "Qode/Qontrols/QurvePresets/Qutton.h"

#include "Qode/Qontrols/QUI/Qrid.h"
#include "Qode/Qontrols/QUI/QSVG.h"
#include "Qode/Qontrols/QUI/Sqope.h"
#include "Qode/Qontrols/QUI/Qabel.h"
#include "Qode/Qontrols/QUI/Qubble.h"
#include "Qode/Qontrols/QUI/Baqground.h"
#include "Qode/Qontrols/QUI/Qinformation.h"
#include "Qode/Qontrols/QUI/QornerResizer.h"
...
: Plugin(_qinfo, MakeConfig(qNumParams, qNumPresets))
{
  ...
  GetParam(qFrequency)->InitDouble("frequency", 20., .0, 40., .001, "Hz");
  ...
  mLayoutFunc = [&](IGraphics* _qui) {
    ...
    auto saveQutton = [](IGraphics& _qui, const IRECT& _bounds, const IColor& _c, float _smoothGrow) {
      ...
      _qui.DrawLine(_c, r.L, r.T, r.R - spac, r.T, 0, thickness);
      _qui.DrawLine(_c, r.R - spac, r.T, r.R, r.T + spac, 0, thickness);
      _qui.DrawLine(_c, r.R, r.T + spac, r.R, r.B, 0, thickness);
      _qui.DrawLine(_c, r.R, r.B, r.L, r.B, 0, thickness);
      _qui.DrawLine(_c, r.L, r.B, r.L, r.T, 0, thickness);

      _qui.DrawLine(_c, r.L + spac, r.T, r.L + spac, r.T + r.H() / 3.f, 0, thickness);
      _qui.DrawLine(_c, r.L + spac, r.T + r.H() / 3.f, r.R - spac, r.T + r.H() / 3.f, 0, thickness);
      _qui.DrawLine(_c, r.R - spac, r.T + r.H() / 3.f, r.R - spac, r.T, 0, thickness);

      _qui.DrawLine(_c, r.L + r.W() / 9, r.B, r.L + r.W() / 9, r.B - r.H() / 2.5f, 0, thickness);
      _qui.DrawLine(_c, r.L + r.W() / 9, r.B - r.H() / 2.5f, r.R - r.W() / 9, r.B - r.H() / 2.5f, 0, thickness);
      _qui.DrawLine(_c, r.R - r.W() / 9, r.B - r.H() / 2.5f, r.R - r.W() / 9, r.B, 0, thickness);
    };
    ...
    auto randomizeQutton = [](IGraphics& _qui, const IRECT& _bounds, const IColor& _c, float _smoothGrow) {
      ...
      float size = r.W() / 10;
      float thickness = .81f;
      const IColor c = _c.WithOpacity(thickness);

      _qui.DrawLine(_c, r.L + size, r.T + size * 2.5f, r.MW(), r.T + size, 0, thickness);
      _qui.DrawLine(_c, r.MW(), r.T + size, r.R - size, r.T + size * 2.5f, 0, thickness);
      _qui.DrawLine(_c, r.L + size, r.T + size * 2.5f, r.L + size, r.B - size * 2.5f, 0, thickness);
      _qui.DrawLine(_c, r.L + size, r.B - size * 2.5f, r.MW(), r.B - size, 0, thickness);
      _qui.DrawLine(_c, r.MW(), r.B - size, r.R - size, r.B - size * 2.5f, 0, thickness);
      _qui.DrawLine(_c, r.R - size, r.B - size * 2.5f, r.R - size, r.T + size * 2.5f, 0, thickness);

      _qui.DrawLine(_c, r.L + size, r.T + size * 2.5f, r.MW(), r.T + size * 4, 0, thickness);
      _qui.DrawLine(_c, r.MW(), r.T + size * 4, r.R - size, r.T + size * 2.5f, 0, thickness);
      _qui.DrawLine(_c, r.MW(), r.T + size * 4, r.MW(), r.B - size, 0, thickness);
    ...
    auto smoothQQutton = [](IGraphics& _qui, const IRECT& _bounds, const IColor& _c, float _smoothGrow) {
      float l = _bounds.GetLengthOfShortestSide();
      const IRECT r = IRECT::MakeXYWH(_bounds.MW() - l / 2, _bounds.MH() - l / 2, l, l).GetScaledAboutCentre(_smoothGrow);
      float size = 3 * r.W() / 9;
      ...
      _qui.FillRoundRect(_c.WithOpacity(.0673f), _bounds.GetScaledAboutCentre(.94f), 3.f);
      ...
      _qui.PathStroke(_c, thickness);

      _qui.DrawRoundRect(_c, tR, .67f, 0, thickness);
      _qui.DrawRoundRect(_c, bL, .67f, 0, thickness);
    };
    ...
    auto linearQQutton = [&](IGraphics& _qui, const IRECT& _bounds, const IColor& _c, float _smoothGrow) {
      float l = _bounds.GetLengthOfShortestSide();
      const IRECT r = IRECT::MakeXYWH(_bounds.MW() - l / 2, _bounds.MH() - l / 2, l, l).GetScaledAboutCentre(_smoothGrow);
      float size = 3 * r.W() / 9;
      const IRECT tR = r.GetFromTRHC(size, size).GetFromBLHC(size / 1.7f, size / 1.7f); // topRight
      ...
      _qui.DrawLine(_c, tR.L, tR.B, bL.R, bL.T, 0, thickness);
      _qui.DrawRoundRect(_c, tR, .67f, 0, thickness);
      _qui.DrawRoundRect(_c, bL, .67f, 0, thickness);
    };
    ...
    auto flipVQutton = [](IGraphics& _qui, const IRECT& _bounds, const IColor& _c, float _smoothGrow) {
      float l = _bounds.GetLengthOfShortestSide();
      const IRECT r = IRECT::MakeXYWH(_bounds.MW() - l / 2, _bounds.MH() - l / 2, l, l).GetScaledAboutCentre(_smoothGrow);
      float sizeX = r.W() / 20;
      float sizeY = r.H() / 8;
      ...
      _qui.DrawLine(_c, line1A.first, line1A.second, line1B.first, line1B.second, 0, sizeX);
      _qui.DrawLine(_c, line2A.first, line2A.second, line2B.first, line2B.second, 0, sizeX);
      ...
      _qui.DrawTriangle(_c, t1VertxA.first, t1VertxA.second, t1VertxB.first, t1VertxB.second, t1VertxC.first, t1VertxC.second, 0, sizeX);
      _qui.DrawTriangle(_c, t2VertxA.first, t2VertxA.second, t2VertxB.first, t2VertxB.second, t2VertxC.first, t2VertxC.second, 0, sizeX);
    };
...
//*****************************************************************************************************************************************
void Quiet::OnParamChangeUI(int _paramIdx, EParamSource _ep)
{
  if (_paramIdx == qSnap)
  {
    const IGraphics* ui = GetUI();
  ...
  else if (_paramIdx == qTheme)
  {
    ...
    if (ui)
    {
      ...
      if (pum)
      {
        ...
        if (pum->GetState() == pum->kExpanded && _ep != EParamSource::kUI)
...
//*****************************************************************************************************************************************
void Quiet::ProcessBlock(sample** _ins, sample** _outs, int _frames)
{
  bool showScope = GetUI() && GetParam(qWaveform)->Bool();

  showSidechain = IsChannelConnected(ERoute::kInput, 2);
  transporting = qp.processBlock(_ins, _outs, _frames, mTimeInfo, showSidechain, showScope);
}
...
//*****************************************************************************************************************************************
int Quiet::UnserializeState(const IByteChunk& _chunk, int _startPos)
{
  std::string qName;
  char sentinel;
  int qQnodeCount;
  ...
  _startPos = _chunk.Get(&nameSize, _startPos);

  for (int i = 0; i < nameSize; i++)
  {
    _startPos = _chunk.Get(&sentinel, _startPos);
    qName.push_back(sentinel);
  }

  qurve.setQurveName(qName.c_str());
  _startPos = _chunk.Get(&qQnodeCount, _startPos);
  qurve.setQnodeCount(qQnodeCount);
  _startPos = _chunk.Get(&qSmooth, _startPos);
  ...
  for (int j = 0; j < qQnodeCount; j++)
  {
    ...
    _startPos = _chunk.Get(&qQurvature, _startPos);
    qurve.setQurvature(j, qQurvature);
    _startPos = _chunk.Get(&qQnode, _startPos);
    qurve.setQnode(j, qQnode);
  }

  qurve.qreateQurve();

  return UnserializeParams(_chunk, _startPos);
}
...
void Quiet::updateLookAhead()
{
  ...
  case 1: // 10 ms
    ...
    qurve.setQurveMultiplier(1.25f);
