...
: Plugin(_qinfo, MakeConfig(qNumParams, qNumPresets))
{
  ...
  GetParam(qTheme)->InitEnum("theme", 1, 5, "", 0, "", "red", "blue", "light", "green", "yellow");
  ...
  GetParam(qFrequency)->InitDouble("frequency", 10, 0.1, 19.9, 0.001, "Hz"); // 20 Hz has GUI issues
  GetParam(qPhaseShift)->InitInt("phase", 0, 0, 360, "°");

  GetParam(qDepth)->InitDouble("depth", 100, 0, 100, 0.001);
  GetParam(qDepth)->SetDisplayPrecision(0);

  GetParam(qSmoothing)->InitDouble("smoothing", 0, 0, 50, 0.01, "ms");
  ...
  mLayoutFunc = [&](IGraphics* _qui) {
    ...
    _qui->AttachControl(new Baqground(bounds, qheme), QtrlTagBaq);
    ...
    _qui->AttachControl(new QSVG(bounds.GetPadded(0, -33, -580, -346).GetPadded(-2), qheme, quietLogo, [_qui](IControl*) {
      // Display About Info
      _qui->GetControlWithTag(QtrlTagInfo)->Hide(false);
    }, true));
    ...
    _qui->AttachControl(new Sqope(windowBounds, qheme, qWaveform, qp.getPhase(), showSidechain, qp.avoidInputs(), qp.getIns(), qp.getOuts(), qp.getSc()), QtrlTagSqope);
    _qui->AttachControl(new QnodeSystem(windowBounds, qurve, qheme, [&](IControl*) {
      ...
      if (addAsterisk(name))
      {
        ...
        ui->GetControlWithTag(QtrlTagQenu)->As<Qenu>()->setPresetName(name.c_str(), true);
      }
    }), QtrlTagQnodeSys);
    ...
    _qui->AttachControl(new UndoQutton(undoBounds.GetPadded(-1), qheme, [&](IControl*) {
      ...
      ui->GetControlWithTag(QtrlTagQnodeSys)->SetDirty(false);
    }), QtrlTagUndo);
    ...
    _qui->AttachControl(new QuttonA(presetsBounds.GetPadded(-115, 0, -320, 0).GetPadded(-1), qheme, "<", [_qui](IControl*) {
      // Previous preset
      _qui->GetControlWithTag(QtrlTagQenu)->As<Qenu>()->loadPreviousPreset();
    }));
    ...
    _qui->AttachControl(new QuttonA(presetsBounds.GetPadded(-320, 0, -115, 0).GetPadded(-1), qheme, ">", [_qui](IControl*) {
      // Next preset
      _qui->GetControlWithTag(QtrlTagQenu)->As<Qenu>()->loadNextPreset();
    }));
    ...
    auto saveQutton = [](IGraphics& _qui, const IRECT& _bounds, const IColor& _color, float _smoothGrow) {
      ...
      float spac = r.W() / 5;
      float strokeWidth = 0.81f;

      _qui.DrawLine(_color, r.L, r.T, r.R - spac, r.T, 0, strokeWidth);
      _qui.DrawLine(_color, r.R - spac, r.T, r.R, r.T + spac, 0, strokeWidth);
      _qui.DrawLine(_color, r.R, r.T + spac, r.R, r.B, 0, strokeWidth);
      _qui.DrawLine(_color, r.R, r.B, r.L, r.B, 0, strokeWidth);
      _qui.DrawLine(_color, r.L, r.B, r.L, r.T, 0, strokeWidth);

      _qui.DrawLine(_color, r.L + spac, r.T, r.L + spac, r.T + r.H() / 3, 0, strokeWidth);
      _qui.DrawLine(_color, r.L + spac, r.T + r.H() / 3, r.R - spac, r.T + r.H() / 3, 0, strokeWidth);
      _qui.DrawLine(_color, r.R - spac, r.T + r.H() / 3, r.R - spac, r.T, 0, strokeWidth);

      _qui.DrawLine(_color, r.L + r.W() / 9, r.B, r.L + r.W() / 9, r.B - r.H() / 2.5f, 0, strokeWidth);
      _qui.DrawLine(_color, r.L + r.W() / 9, r.B - r.H() / 2.5f, r.R - r.W() / 9, r.B - r.H() / 2.5f, 0, strokeWidth);
      _qui.DrawLine(_color, r.R - r.W() / 9, r.B - r.H() / 2.5f, r.R - r.W() / 9, r.B, 0, strokeWidth);
    };
    _qui->AttachControl(new QuttonB(presetsBounds.GetPadded(-356, 0, -81, 0).GetPadded(-3), qheme, saveQutton, [_qui](IControl*) {
      // Save preset
      _qui->GetControlWithTag(QtrlTagQenu)->As<Qenu>()->savePreset();
    }));
    ...
    auto randomizeQutton = [](IGraphics& _qui, const IRECT& _bounds, const IColor& _color, float _smoothGrow) {
      ...
      const IColor c = _color.WithOpacity(0.81f);
      float size = r.W() / 10;
      float strokeWidth = 0.94f;

      _qui.DrawLine(_color, r.L + size, r.T + size * 2.5f, r.MW(), r.T + size, 0, strokeWidth);
      _qui.DrawLine(_color, r.MW(), r.T + size, r.R - size, r.T + size * 2.5f, 0, strokeWidth);
      _qui.DrawLine(_color, r.L + size, r.T + size * 2.5f, r.L + size, r.B - size * 2.5f, 0, strokeWidth);
      _qui.DrawLine(_color, r.L + size, r.B - size * 2.5f, r.MW(), r.B - size, 0, strokeWidth);
      _qui.DrawLine(_color, r.MW(), r.B - size, r.R - size, r.B - size * 2.5f, 0, strokeWidth);
      _qui.DrawLine(_color, r.R - size, r.B - size * 2.5f, r.R - size, r.T + size * 2.5f, 0, strokeWidth);

      _qui.DrawLine(_color, r.L + size, r.T + size * 2.5f, r.MW(), r.T + size * 4, 0, strokeWidth);
      _qui.DrawLine(_color, r.MW(), r.T + size * 4, r.R - size, r.T + size * 2.5f, 0, strokeWidth);
      _qui.DrawLine(_color, r.MW(), r.T + size * 4, r.MW(), r.B - size, 0, strokeWidth);

      _qui.FillEllipse(c, r.MW(), r.T + size * 2.5f, size / 2, size / 3, 0);

      _qui.FillEllipse(c, r.L + size * 2, r.T + size * 4, size / 2, size / 2.5f, 60);
      _qui.FillEllipse(c, r.L + size * 4, r.B - size * 2.5f, size / 2, size / 2.5f, 60);

      _qui.FillEllipse(c, r.R - size * 2, r.T + size * 4, size / 2, size / 2.5f, -60);
      _qui.FillEllipse(c, r.R - size * 4, r.T + size * 5, size / 2, size / 2.5f, -60);
      _qui.FillEllipse(c, r.R - size * 2, r.B - size * 3.5f, size / 2, size / 2.5f, -60);
      _qui.FillEllipse(c, r.R - size * 4, r.B - size * 2.5f, size / 2, size / 2.5f, -60);
    };
    ...
    auto smoothQQutton = [](IGraphics& _qui, const IRECT& _bounds, const IColor& _color, float _smoothGrow) {
      ...
      float strokeWidth = r.W() / 18;
      float sineLike = strokeWidth / 2;

      _qui.FillRoundRect(_color.WithOpacity(0.0673f), _bounds.GetScaledAboutCentre(0.94f), 3);
      ...
      _qui.PathStroke(_color, strokeWidth);

      _qui.DrawRoundRect(_color, tR, 0.67f, 0, strokeWidth);
      _qui.DrawRoundRect(_color, bL, 0.67f, 0, strokeWidth);
    };
    ...
    auto linearQQutton = [&](IGraphics& _qui, const IRECT& _bounds, const IColor& _color, float _smoothGrow) {
      ...
      _qui.DrawLine(_color, tR.L, tR.B, bL.R, bL.T, 0, strokeWidth);
      _qui.DrawRoundRect(_color, tR, 0.67f, 0, strokeWidth);
      _qui.DrawRoundRect(_color, bL, 0.67f, 0, strokeWidth);
    };
    ...
    auto flipVQutton = [](IGraphics& _qui, const IRECT& _bounds, const IColor& _color, float _smoothGrow) {
      ...
      _qui.DrawLine(_color, line1A.first, line1A.second, line1B.first, line1B.second, 0, strokeWidth);
      _qui.DrawLine(_color, line2A.first, line2A.second, line2B.first, line2B.second, 0, strokeWidth);
      ...
      _qui.DrawTriangle(_color, t1VertxA.first, t1VertxA.second, t1VertxB.first, t1VertxB.second, t1VertxC.first, t1VertxC.second, 0, strokeWidth);
      _qui.DrawTriangle(_color, t2VertxA.first, t2VertxA.second, t2VertxB.first, t2VertxB.second, t2VertxC.first, t2VertxC.second, 0, strokeWidth);
    };
    ...
    const IRECT rateQabelBounds = bounds.GetPadded(0, -194, -580, -200).GetPadded(-2);
    _qui->AttachControl(new Qabel(rateQabelBounds, qheme, "rate"), kNoTag, "tempo");
    _qui->AttachControl(new Qlider(rateBounds.GetPadded(-13, -9, -13, -9), qheme, qRate, displayValue), kNoTag, "tempo");


    // Frequency (Free) Slider
    const IRECT freqBounds = bounds.GetPadded(0, -276, -580, -99).GetPadded(-1);
    const IRECT freqQabelBounds = bounds.GetPadded(0, -265, -580, -129).GetPadded(-2);
    _qui->AttachControl(new Qabel(freqQabelBounds, qheme, "frequency"), kNoTag, "free");
    _qui->AttachControl(new Qlider(freqBounds.GetPadded(-13, -9, -13, -9), qheme, qFrequency, displayValue), kNoTag, "free");
    ...
    _qui->AttachControl(new Qnob(nullBounds.GetPadded(-23), qheme, kNoParameter, nullptr))->SetDisabled(true);


    // Depth (Dry/Wet) Knob
    const IRECT depthBounds = bounds.GetPadded(-305, -321, -305, -4);
    const IRECT depthQabelBounds = bounds.GetPadded(-305, -312, -305, -86);
    ...
    // Other...
    _qui->SetScaleConstraints(0.8f, 1.5f);
    ...
    _qui->AttachControl(new Qinformation(bounds.GetPadded(-1), qheme), QtrlTagInfo)->Hide(true);
  };
}
...
void Quiet::OnIdle()
{
  if (transporting)
  {
    ...
    if (ui)
      ui->GetControlWithTag(QtrlTagSqope)->SetDirty(false);
  }
}
...
void Quiet::OnReset()
{
  qp.onReset(GetSampleRate(), GetLatency());
}
...
void Quiet::OnParamChangeUI(int _paramIdx, EParamSource _eps)
{
  if (_paramIdx == qSnap)
  {
    ...
    if (ui)
    {
      QnodeSystem* qs = ui->GetControlWithTag(QtrlTagQnodeSys)->As<QnodeSystem>();
  ...
  else if (_paramIdx == qTheme)
  {
    ...
    if (ui)
    {
      ...
      qheme.updateQheme(static_cast<Qheme::QhemeType>(GetParam(qTheme)->Int()));
...
void Quiet::updateLookAhead()
{
  ...
  case 3: // 30 ms
    SetLatency(1323);
    qp.setLookAhead(1323u);
    qp.setLookingAhead(true);
    qurve.setMultiplyQurvature(true);
    qurve.setQurvatureMultiplier(2.0f);
    break;

  case 4: // 40 ms
    SetLatency(1764);
    qp.setLookAhead(1764u);
    qp.setLookingAhead(true);
    qurve.setMultiplyQurvature(true);
...
{
  _qui->AttachPopupMenuControl(IText(16, EAlign::Center, qheme.foreGround));
  _qui->GetPopupMenuControl()->SetItemColor(qheme.foreGround);
  _qui->GetPopupMenuControl()->SetPanelColor(qheme.grayGround.WithOpacity(0.23f));
  _qui->GetPopupMenuControl()->SetItemMouseoverColor(qheme.grayGround.WithContrast(0.23f));
  _qui->GetPopupMenuControl()->SetCellBackgroundColor(qheme.foreGround.WithOpacity(0.073f));
}

//*****************************************************************************************************************************************
void Quiet::updateUndoRedoQuttons(const IGraphics* _qui)
{
  _qui->GetControlWithTag(QtrlTagUndo)->SetDisabled(!qurve.undoPossible());
  _qui->GetControlWithTag(QtrlTagRedo)->SetDisabled(!qurve.redoPossible());
}